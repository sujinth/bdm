    ...
    async function getResources(){
        try{
          ...
          if(response.data.result?.length !==0 ){
            let filterResourcesList = response.data?.result?.filter((item)=> item?.type == 'resourcelist');
    ...
    return (
        ...
        <div className={`${Styles.container} ${Styles.innerpgcntnt} `}>
            ...
            <div className={Styles.listitems}>
                {resorcesList?.length !==0 && <ul className={Styles.listcntnt}>
                    {resorcesList?.map((item,idx)=>(
                     <li key={idx}><Link href={`/resources/${item?.id}`}>{item.name}</Link></li> 
                    ))}
                </ul>}
            </div>
            </div>
