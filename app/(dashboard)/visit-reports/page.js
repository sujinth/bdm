                ...
                <div className={Styles.bgcolor}>
                    <div className={`${Styles.container} ${Styles.innerpgcntnt}`}>
                        ...
                        <div className={Styles.visitnamebx}>
                            ...
                            {!isReportsSelected && goBackToPage.pageTwo && Object.keys(dealers).length !==0 !== 0 &&<Tabs
                                id="controlled-tab-example"
                                className="tabbtn"
                            >
                               {dealers.dealergroup && <Tab eventKey="DealerGroup" title={dealers.dealergroup  ? "Dealer Group" : ''}>
                                    <div className={`${Styles.listitems}`}>
                                        <div className={Styles.listtoptitle}>My Dealers Group</div>
                                        <ul className={Styles.listcntnt}>
                                            {dealers.dealergroup?.[0]?.dealergroup?.map((item,index)=>(
                                                <li key={index} onClick={() => handleClickDealer(item,'Y')}>
                                ...
                                {dealers?.dealership && <Tab eventKey="Dealer" title={ dealers.dealership ? "Dealer" : ''}>
                                    <div className={`${Styles.listitems}`}> 
                                        <div className={Styles.listtoptitle}>My Dealers</div>
                                        <ul className={Styles.listcntnt}>
                                            {dealers?.dealership?.map((item,index)=>(
                                                <li key={index} onClick={() => handleClickDealer(item,'N')}>
                                                        {item?.name}
                                                </li>
                                            ))}
                                        </ul>
