//////////////////////////////////////////////////////////////////////////////////
//                                                                              //
//        File for showing template form functionality                          //
//                                                                              //
//////////////////////////////////////////////////////////////////////////////////
...
const VisitReportTemplate = ({ selectedData }) => {
  ...
  useEffect(() => {
    if (formData?.flagTabbedView === "N") {
      ...
      const txtDateTimeElement = document.querySelector(
        'input[name="txtDateTime"]'
      );
      const txtDealershipNameElement = document.querySelector(
        'input[name="txtDealershipName"]'
      );
  ...
  useEffect(() => {
    ...
    session.data?.user?.id,
    goBackToPage.pageFour,
    selectedExistingVisitReportData,
  ]);
  ...
  useEffect(() => {
    if (
      (isActiveFollowUpReport && goBackToPage.pageFour) ||
      (isActiveFollowUpReport && formData.flagTabbedView == "N")
    ) {
  ...
  const fetchFormElementValues = async () => {
    ...
    for (let i = 0; i < cleanIds.length; i++) {
      ...
      if (elementId.substring(0, 3) === "spn") {
        ...
        for (let j = 1; j <= arrLength; j++) {
          let chkBox =
            document.getElementById(`${elementId}${j}`)?.checked ?? false;
          if (chkBox) {
            eleId = `${elementId}@@${j},`;
        ...
        for (let j = 1; j <= arrLength; j++) {
          ...
          if (element?.checked) {
            ...
            // elementId
            elementContent =
              document.getElementById(`${elementId}${j}`)?.value || "";
            break;
          } else {
            eleId = `${elementId}@@${cleanIds[i][1]},`;
          }
    ...
    let totalquestioncount =
      document.getElementById("totalquestioncount")?.value || "";

    // Case  flagTabbedView === 'N'
    if (formData?.flagTabbedView === "N") {
      txtDateTimeElement =
        document.querySelector('input[name="txtDateTime"]')?.value || "";
      lstDealershipStatus =
        document.getElementById("lstDealershipStatus")?.value || "";
      packExpiryDate =
        document.getElementById("txtPackExpiryDate")?.value || "";
    ...
    if (formData?.flagTabbedView === "Y") {
      txtDealerSignature =
        document.getElementById("txtDealerSignature")?.value || "";
  ...
  const handleSubmit = async (sendEmailFlag) => {
    try {
      ...
      let response = await axios.post(
        "/api/visitReports/postDealershipVisitReport",
        formDatas
      );
  ...
  const handleLandingFormSaveButton = (e) => {
    try {
      ...
      let storedSavedData =
        JSON.parse(localStorage.getItem("visitReportForm")) || {};
  ...
  async function getVisitReportList() {
    try {
      ...
      const response = await axios.post(
        `/api/visitReports/visitReportsList?userId=${userId}`,
        requestBodyObj
      );
  ...
  const handleSelectExistingVisitReportData = (e, item) => {
    ...
    let formattedReviewDate = item.reviewdate
      ? moment(item.reviewdate, "DD/MM/YYYY").format("YYYY-MM-DD")
      : "";
      ...
      if (recipients.recipientList.length !== 0) {
        ...
        let requestBodyObj = {
          body: {
            doAction: "sendMail",
            userid: userId,
            dealershipVisitreportCartId1:
              selectedExistingVisitReportData?.cartid || "",
  ...
  const setSavedFormElementValues = async (
    formControlId,
    formControlResult
  ) => {
    try {
          ...
          if (elementId.substring(0, 3) === "spn") {
            ...
            if (element) {
              ...
              if (element.value !== "0") {
                ...
                let chkElement = document.getElementsByName(
                  `chk_${hidElementNum?.[1]}[]`
                );
  ...
  const initiallyFetchSavedFormDataStorage = async () => {
    try {
      ...
      if (
        Object.keys(storedSavedData)?.length !== 0 &&
        Object.keys(selectedExistingVisitReportData).length == 0 &&
        formId
      ) {
        if (storedSavedData[userId] && storedSavedData[userId][formId]) {
          ...
          // Tabbed view = false
          if (flagTabbedView == "N") {
            const dealershipStatusElement = document.getElementById(
              "lstDealershipStatus"
            );
            const packExpiryDateElement =
              document.getElementById("txtPackExpiryDate");
            const attendeesElement = document.getElementById("txtAttendees");
            if (dealershipStatusElement) {
              dealershipStatusElement.value =
                savedData?.lstDealershipStatus || "";
            }
          ...
          // Tabbed view = true
          if (flagTabbedView == "Y") {
            const txtDealerSignature =
              document.getElementById("txtDealerSignature");
    ...
    try {
      ...
      if (storedSavedData?.[userId]) {
        ...
        localStorage.setItem(
          "visitReportForm",
          JSON.stringify(storedSavedData)
        );
        setFormValues((prev) => ({
          ...prev,
          dealerAttendees: "",
      ...
      <div className={`${Styles.container} ${Styles.innerpgcntnt}`}>
          ...
          <div className={Styles.listitems}>
            <ul className={`${Styles.listcntnt} ${Styles.listiconhide}`}>
              <li
                ...
                ref={liTagNewFormRef}
                className={`${Styles.listhead} ${Styles.toplist}`}
              >
                <a>New Form</a>
              </li>
              {loaderInSideBar ? (
                  ...
                  <ul>
                    {visitReportList?.map((item, index) => (
                      <li
                        key={index}
                        onClick={(e) =>
                          handleSelectExistingVisitReportData(e, item)
                        }
                      >
        ...
        <div className={Styles.detailbx}>
          <div className={Styles.titlebx}>
            {`${selectedDealer?.name || selectedDealer?.dealerGroupName} - ${
              selectedReportData?.formName
            }`}
          </div>
          <div
            className={`${Styles.contentwhtbx} ${Styles.contentwhtbxfooter}`}
          >
            {/* Visit report middle form */}
            {!goBackToPage.pageFour &&
              selectedReportData.flagTabbedView == "Y" && (
              ...
              >
                ...
                !isActiveFollowUpReport ? (
                  <div className={`${Styles.flex} ${Styles.rowrhtbtn}`}>
                    ...
                    {Object.keys(selectedExistingVisitReportData).length !==
                      0 &&
                      selectedReportData.flagTabbedView == "N" &&
                      !isActiveFollowUpReport && (
                        <CustomButton
                  ...
                  <div className={`${Styles.flex} ${Styles.rowrhtbtn}`}>
                    ...
                    <CustomButton
                      type="button"
                      onClick={() => {
                        handleTemplateFormCancelButton(true);
                      }}
                    >
                      Cancel
                    </CustomButton>
                ...
                {formData.flagImage === "Y" && (
                  <div className={Styles.imageaddoption}>
                      ...
                      overlay={
                        <Popover id={`addImage-popover`}>
                          <Popover.Body className={Styles.imagepopover}>
                            {/* No image found */}
                            {Object.keys(selectedExistingVisitReportData)
                              .length !== 0 &&
                              !isActiveFollowUpReport &&
                    ...
                    >
                      ...
                      >
                        {Object.keys(selectedExistingVisitReportData).length !==
                          0 && !isActiveFollowUpReport
                          ? `View Images`
                          : `Add Images`}{" "}
                        <img src="../../addimage.svg" alt="message.svg" />
                      </Button>
                    </OverlayTrigger>
                  </div>
                )}
                    ...
                    <>
                      <div className={Styles.searchbox}>
                        <PopoverComponent
                          ...
                          label={
                            Object.keys(selectedExistingVisitReportData)
                              .length !== 0 && !isActiveFollowUpReport
                              ? `View Recipients`
                              : `Add Recipients`
                          }
...
function VisitReportForm({
  ...
  formData,
  isReadOnly,
  handleFollowUpReport,
  isAllreadyExistVisitReport,
  isActiveFollowUpReport,
  handleMiddleFormCancelButton,
}) {
  return (
    <>
      <form>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                <input
                  className={Styles.tblinputbx}
                  value={
                    formData.flagHealthCheck === "Y"
                      ? formValues.txtDealershipName
                      : formValues.reviewPeriod
                  }
                  readOnly={
                    formData.flagHealthCheck == "Y" ||
                    (isAllreadyExistVisitReport &&
                      formData.flagHealthCheck == "N" &&
                      !isActiveFollowUpReport)
                  }
                  type="text"
                  onChange={(e) =>
            ...
            <tr>
              ...
              <td>
                <textarea
                  ...
                  readOnly={isReadOnly}
                  onChange={(e) =>
                    handleFormChange("dealerAttendees", e.target.value)
                  }
                />
              </td>
            </tr>
            <tr>
              ...
              <td>
                <textarea
                  ...
                  readOnly={isReadOnly}
                  onChange={(e) =>
                    handleFormChange("scukAttendees", e.target.value)
                  }
                />
              </td>
            </tr>
            ...
            <div className={Styles.mainboxfooter}>
              ...
              >
                {/* Action Buttons */}
                <div className={`${Styles.flex} ${Styles.rowrhtbtn}`}>
                  {isAllreadyExistVisitReport && !isActiveFollowUpReport && (
                    <CustomButton
                      type="button"
                      onClick={(e) => handleFollowUpReport()}
                    >
                  ...
                  <CustomButton type="button" onClick={handleContinueButton}>
                    Continue
                  </CustomButton>
                </div>
              </div>
...
const PopoverComponent = ({
  ...
  const addRecipient = (e) => {
    try {
      ...
      if (updatedList.includes(inputValue)) {
        ...
        setRecipients((prev) => ({ ...prev, [listName]: updatedList }));
        setInputValue("");
        setErrorMessage("");
      }
    } catch (err) {
  ...
  const removeRecipient = (recipient) => {
    ...
    setRecipients((prev) => ({ ...prev, [listName]: filteredData }));
  };
